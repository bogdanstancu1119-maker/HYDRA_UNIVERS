#!/usr/bin/env python3
"""
hydra-psie-bursa-v FINAL UNIFICAT v3.1 (Corectat)
...
def load_config():
    if CONFIG_FILE.exists():
        try:
            ...
            logging.error(f"Eroare la citirea fișierului de configurare: {e}")
    return {
        "praguri": [
            [500, 100],
            [5000, 1000],
...
def aplica_stop_loss(capital, capital_max):
    if capital_max > 0 and capital < capital_max * (1 - STOP_LOSS_PCT):
...
def aplica_stop_loss_global(capital, capital_max):
    if capital_max > 0 and capital < capital_max * (1 - STOP_LOSS_GLOBAL_PCT):
...
def valideaza_simbol(simbol):
    simbol_up = simbol.upper()
    if simbol_up not in SIMBOLURI_PERMISE:
...
def preia_date_piata_reale(simbol="BTCUSDT", reincercari=2):
...
def genereaza_raport(m):
    ...
    try:
        ...
        cap_ultim = m.get('istoric', [{}])[-1].get('capital', 0) if m.get('istoric') else 0
        raport += f"- **Capital Viu:** {cap_ultim:.2f}€\n"
